...
if ( ! function_exists( 'wbsPress_start_cleanup' ) ) :
function wbsPress_start_cleanup() {

	// Launching operation cleanup.
	add_action( 'init', 'wbsPress_cleanup_head' );

	// Remove WP version from RSS.
	add_filter( 'the_generator', 'wbsPress_remove_rss_version' );

	// Remove pesky injected css for recent comments widget.
	add_filter( 'wp_head', 'wbsPress_remove_wp_widget_recent_comments_style', 1 );

	// Clean up comment styles in the head.
	add_action( 'wp_head', 'wbsPress_remove_recent_comments_style', 1 );

	// Clean up gallery output in wp.
	add_filter( 'wbsPress_gallery_style', 'wbsPress_gallery_style' );

}
add_action( 'after_setup_theme','wbsPress_start_cleanup' );
endif;
/**
 * Clean up head.+
 * ----------------------------------------------------------------------------
...
if ( ! function_exists( 'wbsPress_cleanup_head' ) ) :
function wbsPress_cleanup_head() {
	...
	add_filter( 'style_loader_src', 'wbsPress_remove_wp_ver_css_js', 9999 );

	// Remove WP version from scripts.
	add_filter( 'script_loader_src', 'wbsPress_remove_wp_ver_css_js', 9999 );

}
endif;

// Remove WP version from RSS.
if ( ! function_exists( 'wbsPress_remove_rss_version' ) ) :
function wbsPress_remove_rss_version() { return ''; }
endif;

if ( ! function_exists( 'wbsPress_remove_wp_ver_css_js' ) ) :

// Remove WP version from scripts.
function wbsPress_remove_wp_ver_css_js( $src ) {
	if ( strpos( $src, 'ver=' ) ) {
...
if ( ! function_exists( 'wbsPress_remove_wp_widget_recent_comments_style' ) ) :
function wbsPress_remove_wp_widget_recent_comments_style() {
...
if ( ! function_exists( 'wbsPress_remove_recent_comments_style' ) ) :
function wbsPress_remove_recent_comments_style() {
...
if ( ! function_exists( 'wbsPress_gallery_style' ) ) :
function wbsPress_gallery_style( $css ) {
...
if ( ! class_exists( 'wbsPress_img_rebuilder' ) ) :
	class wbsPress_img_rebuilder {
	...
	$wbsPress_img_rebuilder = new wbsPress_img_rebuilder;
...
add_action('woocommerce_before_main_content', 'wbsPress_before_content', 10);
remove_action( 'woocommerce_after_main_content', 'woocommerce_output_content_wrapper_end', 10);
add_action('woocommerce_after_main_content', 'wbsPress_after_content', 10);
?>
